...
struct wlr_buffer *wlr_swapchain_acquire(struct wlr_swapchain *swapchain) {
    ...
    for (size_t i = 0; i < WLR_SWAPCHAIN_CAP; i++) {
        ...
        if (slot->buffer != NULL) {
            wlr_log(WLR_DEBUG, "Acquiring existing buffer in slot %zu", i);
    ...
    if (free_slot == NULL) {
        ...
        wlr_log(WLR_DEBUG, "Found free slot for new buffer allocation");
    }
    ...
    wlr_log(WLR_DEBUG, "Allocated new buffer for slot");
