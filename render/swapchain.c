...
struct wlr_swapchain *wlr_swapchain_create(
	...
	// Allocate swapchain->format
	swapchain->format = calloc(1, sizeof(*swapchain->format));
	if (swapchain->format == NULL) {
		wlr_log(WLR_ERROR, "Failed to allocate DRM format for swapchain");
		free(swapchain);
		return NULL;
	}
	...
	wlr_log(WLR_DEBUG, "Created swapchain: %dx%d, format=0x%x",
...
struct wlr_buffer *wlr_swapchain_acquire(struct wlr_swapchain *swapchain) {
	...
	if (swapchain->allocator == NULL) {
		wlr_log(WLR_ERROR, "Swapchain allocator is NULL");
		return NULL;
	}
...
void wlr_swapchain_set_buffer_submitted(struct wlr_swapchain *swapchain,
	...
	if (!wlr_swapchain_has_buffer(swapchain, buffer)) {
		wlr_log(WLR_DEBUG, "Buffer not found in swapchain");
		return;
	}
