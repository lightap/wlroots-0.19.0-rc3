...
static const char *ssd_fragment_shader_src =
  "/* Warped noise shader converted to panel format, with curved corners using v_texcoord */\n"
    "#version 300 es\n"
    "precision mediump float;\n"
    "\n"
    "in vec2 v_texcoord; // IMPORTANT: This MUST be correctly supplied for the panel's quad\n"
...
"// It simulates a light from the top-left, making top/left edges brighter\n"
"// and bottom/right edges darker.\n"
"float edgeBumpEffect(vec2 uv) {\n"
"    float bevel_width = 0.01; // The width of the bevel effect\n"
"    float highlight = 10.3;    // How bright the highlight is\n"
"    float shadow = 10.7;       // How dark the shadow is\n"
"\n"
"    // Calculate the minimum distance to a vertical or horizontal edge\n"
"    float min_dist = min(min(uv.x, 1.0 - uv.x), min(uv.y, 1.0 - uv.y));\n"
"\n"
"    // If we are outside the bevel area, there's no lighting effect\n"
"    if (min_dist >= bevel_width) {\n"
"        return 1.0;\n"
"    }\n"
"\n"
"    // Calculate a smooth gradient that is 1.0 at the very edge (min_dist=0)\n"
"    // and fades to 0.0 at the inner side of the bevel (min_dist=bevel_width).\n"
"    float gradient = smoothstep(bevel_width, 0.0, min_dist);\n"
"\n"
"    // Calculate the lighting effect based on a diagonal gradient from top-left to bottom-right.\n"
"    // This averages the effect from the X and Y axes, which handles corners gracefully.\n"
"    float x_effect = mix(highlight, shadow, uv.x); // Light is on the left (uv.x=0)\n"
"    float y_effect = mix(highlight, shadow, uv.y); // Light is on the top (uv.y=0)\n"
"    float light_effect = (x_effect + y_effect) * 0.5; // Average the two effects\n"
"\n"
"    // Blend from no effect (1.0) to the full lighting effect based on the gradient\n"
...
"    }\n"
"    \n"
"    // Step 1: Calculate procedural bump mapping for the edges\n"
"    float bump_factor = edgeBumpEffect(uv);\n"
"    \n"
"    // Step 2: Calculate rounded corner mask\n"
"    float corner_radius = 0.08; // Adjust this value to change corner roundness\n"
"    float corner_mask = roundedCornerMask(uv, corner_radius);\n"
"    \n"
"    // Step 3: Calculate edge anti-aliasing mask\n"
"    float edge_mask = edgeAntiAliasMask(uv);\n"
"    \n"
"    // Apply the bump mapping effect to the color BEFORE applying alpha masks\n"
"    color.rgb *= bump_factor;\n"
"    \n"
"    // Combine corner and edge masks for the final shape\n"
"    float final_mask = min(corner_mask, edge_mask);\n"
"    \n"
"    // Apply combined mask by modifying alpha\n"
"    color.a *= final_mask;\n"
