...
"uniform float u_zoom;               // For brightness/intensity\n"
"uniform highp int u_quadrant;       // Which quadrant to expand (0-3)\n"
"uniform float u_time;               // Time for background animation\n"
"\n"
"in vec2 v_texcoord_to_fs;\n"
"in float v_face_id; // The ID of the cube face being rendered (0-5)\n"
"out vec4 FragColor;\n"
...
"    }\n"
"    \n"
"    // Create multicolored background pattern\n"
"    float time = u_time * 0.3;\n"
"    vec3 bg_color1 = vec3(0.2, 0.1, 0.8);  // Blue\n"
"    vec3 bg_color2 = vec3(0.8, 0.2, 0.4);  // Pink\n"
"    vec3 bg_color3 = vec3(0.1, 0.8, 0.3);  // Green\n"
"    vec3 bg_color4 = vec3(0.9, 0.6, 0.1);  // Orange\n"
"    \n"
"    float blend1 = sin(uv.x * 6.28 + time) * 0.5 + 0.5;\n"
"    float blend2 = cos(uv.y * 6.28 + time * 0.7) * 0.5 + 0.5;\n"
"    \n"
"    vec3 mixed1 = mix(bg_color1, bg_color2, blend1);\n"
"    vec3 mixed2 = mix(bg_color3, bg_color4, blend2);\n"
"    vec3 background = mix(mixed1, mixed2, (sin(time * 2.0) * 0.5 + 0.5));\n"
"    \n"
"    // Blend texture with background (texture dominates, background fills transparent areas)\n"
"    color.rgb = mix(background * 0.3, color.rgb, color.a);\n"
"    color.a = 1.0;\n"
"    \n"
"    // Intensity based on zoom (closer zoom = brighter)\n"
"    float intensity = 1.0; //0.7 + (2.0 - u_zoom) * 0.3;\n"
...
"    #endif\n"
"\n"
"    FragColor = forCol2 + vec4(backCol2, 1.0).bgra;\n"
"}\n";
